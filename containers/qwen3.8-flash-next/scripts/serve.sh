...
#
#   scripts/serve.sh                          # defaults: native 262k ctx, MTP=2
...
#
# Tunables (env):
#   PORT=8000         host port for the API
...
#                     on a 300k prefill with MTP — keep the margin)
...
#   PREFIX_CACHING=0 1 = reuse KV blocks for requests sharing an identical prefix
...
#   IMAGE=dgxt/qwen38-flash-next:latest   MODEL=RadixArk/Qwen3.8-Flash-Next-NVFP4
set -euo pipefail
...
docker run -d --name "$NAME" --restart unless-stopped \
  ...
  "$SNAP_IN" --served-model-name qwen3.8-flash-next \
    ...
    "${PREFIX_ARGS[@]}" --enable-chunked-prefill --max-num-batched-tokens "$BATCH_TOKENS" \
    $CC \
...
echo ">> $NAME starting on :$PORT (model 'qwen3.8-flash-next', ctx $CTX, yarn=$YARN, mtp=$MTP, seqs=$SEQS, prefix_caching=$PREFIX_CACHING)"
